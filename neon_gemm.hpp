...
public:
    struct GemmParams {
        ...
        static constexpr size_t NR = 12;   // 12 столбцов за раз (использует 12 регистров)
    ...
    struct QuantizedResult {
        ...
        QuantizedMatrix to_qmatrix(float output_scale, int32_t output_zp = 0) const;
    };
    
    // Основное умножение A (m×k) × B (k×n) → C (m×n)
    static QuantizedResult multiply(
        const QuantizedMatrix& A,
        const QuantizedMatrix& B,
        const GemmParams& params = GemmParams());
    
private:
    static void micro_kernel_8x12x4(
        const int8_t* A_ptr,
        const int8_t* B_ptr,
        int32_t* C_ptr,
        size_t ldc,
    ...
    static void micro_kernel_simple(
        const int8_t* A_ptr,
        const int8_t* B_ptr,
        int32_t* C_ptr,
    ...
    static void compute_block_neon(
        const QuantizedMatrix& A,
        const QuantizedMatrix& B,
        int32_t* C,
