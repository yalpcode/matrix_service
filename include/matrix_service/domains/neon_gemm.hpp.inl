...
QuantizedMatrix NeonGEMM::QuantizedResult::to_qmatrix(float output_scale, int32_t output_zp = 0) const {
    ...
    for (size_t i = 0; i < data.size(); ++i) {
        ...
        float requantized = dequantized / output_scale + static_cast<float>(output_zp);
...
// Основное умножение
static NeonGEMM::QuantizedResult multiply(
    const QuantizedMatrix& A,
    const QuantizedMatrix& B,
    const NeonGEMM::GemmParams& params = NeonGEMM::GemmParams()) {
...
#if __ARM_NEON
void NeonGEMM::micro_kernel_8x12x4(
    const int8_t* A_ptr,
    const int8_t* B_ptr,
    int32_t* C_ptr,
    size_t ldc,
    ...
    for (int depth = 0; depth < 4; ++depth) {
        ...
        int8x8_t a_col1 = vld1_s8(A_ptr + depth * 8 + 8); // если больше 8, но у нас 8
        
        // Преобразуем в int16, затем в int32 и вычитаем zero_point
        int16x8_t a_col0_s16 = vmovl_s8(a_col0);
        int32x4_t a_low = vsubq_s32(vmovl_s16(vget_low_s16(a_col0_s16)), v_zp_a);
        int32x4_t a_high = vsubq_s32(vmovl_s16(vget_high_s16(a_col0_s16)), v_zp_a);
        ...
        // Для строк 4-7
        a_vecs[1][0] = vdupq_laneq_s32(a_high, 0); // строка 4: 4 копии a_high[0]
        a_vecs[1][1] = vdupq_laneq_s32(a_high, 1); // строка 5: 4 копии a_high[1]
        a_vecs[1][2] = vdupq_laneq_s32(a_high, 2); // строка 6: 4 копии a_high[2]
        a_vecs[1][3] = vdupq_laneq_s32(a_high, 3); // строка 7: 4 копии a_high[3]
...
void NeonGEMM::micro_kernel_simple(
    const int8_t* A_ptr,
    const int8_t* B_ptr,
    int32_t* C_ptr,
...
void NeonGEMM::compute_block_neon(
    const QuantizedMatrix& A,
    const QuantizedMatrix& B,
    int32_t* C,
