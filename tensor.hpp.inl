...
template <IsArithmetic T>
Tensor<T> Tensor<T>::matmul(const Tensor& other, bool quantize = false) const {

    if (quantize == true) {
        if (std::is_same(T, float) == false) {
            throw std::invalid_argument("Quantization is available for float32 only\n");
        }
        QuantizedMatrix A(*this);
        QuantizedMatrix B(other);
        auto result = NeonGEMM::multiply(A, B);
        return result.to_qmatrix().dequantize_to_tensor();
    }
